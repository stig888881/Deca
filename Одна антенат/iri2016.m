...
%   [23] V. Truhl?k, D. Bilitza, and L. T??skov?, "A new global empirical
...
%   [27] L. T??skov?, V. Truhl?k, J. ?milauer, "An empirical model of
...
% We check the entire response first to find the last line of the variable listing,
% currently containing "3-h_kp".
% If this variable is not found, the response is deemed invalid as well as when
% "No data for your input selection" was returned.
% This should guard against future changes in the format or content of the web site
% surrounding the ModelWeb response.
if strfind(result,'No data for your input selection')
error('iri:parseresult:modelWebError', ['The online ' ...
'interface returned: No data for your input selection.']);
end
% find the line number where the numerical output data start
datastart = -1;
newlines = find(result == sprintf('\n'));
for index = 1:length(newlines)-1
if strfind(result(newlines(index)+1:newlines(index+1)-1),'3-h_kp')
datastart=index+3;
end
end
if datastart > 0
data = sscanf(result(newlines(datastart)+1 : newlines(datastart + sweeplen)-1), '%f');
else
error('iri:parseresult:otherModelWebError','The online interface failed');
end